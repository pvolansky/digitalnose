...
from database import connect, utc
...
class PipelineTest(unittest.TestCase):
    def setUp(self):
        ...
        self.db = connect(str(Path(self.temp.name) / 'test.sqlite3'), aggregation_start=self.minute)
    ...
    def raw(self, minute, count=12):
        with self.db:
            self.db.executemany('INSERT INTO sensor_readings(recorded_at_utc,tvoc_ppb,eco2_ppm,aqi,sensor_status) VALUES (?,?,?,?,0)', [(utc(minute+i*5),100+i,600+i,2) for i in range(count)])
    ...
    def test_offline_and_lost_ack_retries_only_minutes(self):
        ...
        self.assertIsNone(self.db.execute('SELECT synced_at_utc FROM minute_aggregates').fetchone()[0])
        def success(url,key,payload):
            self.assertEqual(payload,received[0])
            self.assertNotIn('observed_at',payload)
            self.assertNotIn('synced_at_utc',payload)
            return True
    ...
    def test_batch_limits_and_rejected_rows_do_not_block_later_rows(self):
        ...
        self.assertEqual(self.db.execute('SELECT COUNT(*) FROM minute_aggregates WHERE synced_at_utc IS NULL').fetchone()[0],2)
