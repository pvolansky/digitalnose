...
export default async function Reports({
  ...
  const reports = await loadReports(db, site.id, 50, before);
  return (
    <Shell site={site} sites={sites}>
      ...
      {reports.length === 50 && (
        <Link
          ...
          href={`/report?site=${site.id}&before=${encodeURIComponent(reports.at(-1)!.reported_at)}`}
        >
          Older reports
        </Link>
      )}
      {before && (
        <Link className="button secondary" href={`/report?site=${site.id}`}>
          Latest reports
        </Link>
      )}
