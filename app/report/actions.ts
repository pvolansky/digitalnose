...
export async function reportSmell(_: ActionResult, form: FormData): Promise<ActionResult> {
  let values;
  try {
    values = validateReport(form);
  } catch (e) {
    return { error: e instanceof Error ? e.message : 'Invalid report.' };
  }
