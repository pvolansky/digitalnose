...
export async function siteContext(requested?: string) {
  ...
  const [members, devices] = await Promise.all([
    db.from('site_members').select('role').eq('site_id', site.id).eq('user_id', user.id).single(),
    db.from('devices').select('*').eq('site_id', site.id).order('created_at'),
  ]);
  ...
  return {
    ...
    sites,
    site,
    role: members.data.role as 'owner' | 'resident',
